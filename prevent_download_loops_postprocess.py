...
from loop_prevention_shared import (
    ConfigLoader, LockedFile, Logger, NotifierInterface, create_notifier, ensure_file_exists
)
...
class PostProcessLoopPrevention:
    ...
    def _is_match(self, history_name: str, history_key: str) -> bool:
        ...
        if self.duplicate_key and history_key and self.duplicate_key == history_key:
    ...
    def update_status(self) -> bool:
        ...
        try:
            ...
            if not updated:
                self.log("⚠️ WARNING: No matching PENDING entry found!", "ERROR")
                self.log("This usually means:", "ERROR")
                self.log("  1. Pre-queue script didn't run (check SABnzbd config)", "ERROR")
                self.log("  2. Name/key mismatch between pre-queue and post-process", "ERROR")
                self.log("  3. Entry was already updated or removed", "ERROR")
                # DO NOT add new entry - it causes duplicates with empty fields
                # Instead, log the issue for manual investigation
                self.log("NOT adding new entry to prevent corruption", "ERROR")
