...
class Logger:
    ...
    def __init__(self, log_file: str, max_size_mb: int, max_backups: int, log_level: str) -> None:
        """
        ...
        Args:
            ...
            log_level: Logging level ('ALL', 'ERROR', 'NONE')
        """
    ...
    def log(self, message: str, level: str = "INFO") -> None:
        ...
        if self.log_level == "ERROR" and level != "ERROR":
            return
...
class NotifierInterface(ABC):
    ...
    def send_notification(self, title: str, message: str, priority: Optional[int] = None) -> bool:
        ...
        Args:
            ...
            priority: Optional priority level (implementation-specific)
...
def load_notifier_from_file(filepath: str, config: Dict[str, Any], logger: Optional[Logger] = None) -> Optional[NotifierInterface]:
    ...
    try:
        if not os.path.exists(filepath):
            if logger:
                logger.log(f"Notifier file not found: {filepath}", "ERROR")
            return None
        ...
        if notifier_class is None:
            if logger:
                logger.log(f"{notifier_name}: No NotifierInterface implementation found in file", "ERROR")
            return None
        
        if logger:
            logger.log(f"{notifier_name}: Successfully loaded from {filepath}", "INFO")
        
        return notifier_class(config, logger)
    
    except Exception as e:
        if logger:
            logger.log(f"{notifier_name}: Error loading from file: {e}", "ERROR")
        return None
...
def create_notifier(config: Dict[str, Any], logger: Optional[Logger] = None) -> Optional[NotifierInterface]:
    ...
    if not config_file:
        if logger:
            logger.log(f"{notifier_name}: No config_file specified in notifier configuration", "ERROR")
        return None
